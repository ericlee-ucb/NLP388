...
def main():
    ...
    if args.dataset.endswith('.json') or args.dataset.endswith('.jsonl'):
        ...
        if training_args.do_eval:
            ...
            ds1 = datasets.load_dataset('squad')
            ds2 = datasets.load_dataset('adversarial_qa', 'adversarialQA')
            ds3 = datasets.load_dataset('squad_v2')
            dataset = datasets.DatasetDict({
            'train': interleave_datasets([ds1['train'], ds2['train'], ds3['train']], probabilities=[0.5, 0.25, 0.25], split="train", seed=42, stopping_strategy='all_exhausted'),
            'validation': interleave_datasets([ds1['validation'], ds2['validation'], ds3['validation']], probabilities=[0.5, 0.25, 0.25], split="train", seed=42, stopping_strategy='all_exhausted')})
