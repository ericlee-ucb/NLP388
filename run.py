...
def write_dataset(name, dataset):
    for split, split_dataset in dataset.items():
        split_dataset.to_json(f"{name}-{split}.jsonl")
...
def main():
    ...
    if args.dataset.endswith(".json") or args.dataset.endswith(".jsonl"):
        ...
        eval_split = "train"
    elif args.dataset.endswith("save"):
        print(args.dataset)
        dataset_id = None

        dataset1 = datasets.load_dataset("stanfordnlp/squad_adversarial", "AddSent")
        dataset2 = datasets.load_dataset("squad")
        # write_dataset("squad_adversarial", dataset1)
        # write_dataset("squad", dataset2)

        seed = 42
        probabilities = [0.3, 0.7]
        # datasets = args.dataset
        dataset = datasets.interleave_datasets(
            [dataset1["validation"], dataset2["train"]],
            probabilities=probabilities,
            seed=seed,
        )
        dataset.to_json(f"combined_squad_dataset.jsonl")

    # squad_path = "squad.json"  # Replace with actual path to adversarial dataset
    # adversarial_path = "stanford_adversarial.json"  # Replace with actual path to adversarial dataset
    # output_path = "combined_squad_dataset.json"  # Desired path for combined dataset
    # dataset1.to_json(adversarial_path)
    # dataset2.to_json(squad_path)

    # seed = 42
    # probabilities = [0.3, 0.7]
    # # datasets = args.dataset
    # dataset = datasets.interleave_datasets(
    #     [dataset1["train"], dataset2["train"]],
    #     probabilities=probabilities,
    #     seed=seed,
    # )
